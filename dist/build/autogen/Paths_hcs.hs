module Paths_hcs (
    version,
    getBinDir, getLibDir, getDataDir, getLibexecDir,
    getDataFileName, getSysconfDir
  ) where

import qualified Control.Exception as Exception
import Data.Version (Version(..))
import System.Environment (getEnv)
import Prelude

catchIO :: IO a -> (Exception.IOException -> IO a) -> IO a
catchIO = Exception.catch


version :: Version
version = Version {versionBranch = [0,1], versionTags = []}
bindir, libdir, datadir, libexecdir, sysconfdir :: FilePath

bindir     = "/home/dwat3r/.cabal/bin"
libdir     = "/home/dwat3r/.cabal/lib/i386-linux-ghc-7.6.3/hcs-0.1"
datadir    = "/home/dwat3r/.cabal/share/i386-linux-ghc-7.6.3/hcs-0.1"
libexecdir = "/home/dwat3r/.cabal/libexec"
sysconfdir = "/home/dwat3r/.cabal/etc"

getBinDir, getLibDir, getDataDir, getLibexecDir, getSysconfDir :: IO FilePath
getBinDir = catchIO (getEnv "hcs_bindir") (\_ -> return bindir)
getLibDir = catchIO (getEnv "hcs_libdir") (\_ -> return libdir)
getDataDir = catchIO (getEnv "hcs_datadir") (\_ -> return datadir)
getLibexecDir = catchIO (getEnv "hcs_libexecdir") (\_ -> return libexecdir)
getSysconfDir = catchIO (getEnv "hcs_sysconfdir") (\_ -> return sysconfdir)

getDataFileName :: FilePath -> IO FilePath
getDataFileName name = do
  dir <- getDataDir
  return (dir ++ "/" ++ name)
